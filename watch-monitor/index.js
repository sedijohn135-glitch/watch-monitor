...
const CUSTOM_TOOLS = [
  {
    ...
    annotations: {
      ...
      // readOnlyHint is deliberately true even though this tool writes
      // state (it arms a watch and can fire a Telegram notification).
      // This is an explicit owner choice to stop the client prompting
      // "Allow" on every auto-registration from the sniper pipeline —
      // Stage 14 already auto-arms by skill default (SKILL.md §8), so
      // the human already approved this class of action once at the
      // skill level. It is NOT a claim that the tool has no side
      // effects. If a client is ever pointed at this server without
      // that same skill-level auto-arm understanding in place, flip
      // this back to false so it prompts again.
      readOnlyHint: true,
  ...
  },
  {
    ...
    annotations: {
      ...
      // Same deliberate override as register_watch above: this writes
      // state and can fire a Telegram notification, but auto-arm is
      // already the skill-level default (SKILL.md §8b), so the human
      // already approved this class of action once. Not a claim of
      // no side effects — an explicit choice to stop per-call prompts.
      readOnlyHint: true,
      destructiveHint: false,
      idempotentHint: true,
      openWorldHint: false,
